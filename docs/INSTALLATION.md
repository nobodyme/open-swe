...
> configures `agent_runtime` in production and the `make dev-platform`
> (`langgraph dev`) path locally. `langgraph dev` itself is Elastic-licensed
> and permitted for local development only — it is never the production
> runtime here.
